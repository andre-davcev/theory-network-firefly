...
> extends StateCollection<T, M> {
  ...
  protected static childLookupState(state: any): Record<string, Partial<any>> {
    return state.childLookup;
  }
  ...
  protected static offsetState(state: any): number {
    ...
    return StateChild.keysFilteredState(state).findIndex(
      (id: string) => dataLookup[id] == null
    );
  }
  ...
  public setData(context: StateContext<M>, action: any): Observable<any> {
    ...
    return Object.keys(childLookup).length === 0
      ...
      : of(patchState({ childLookup } as M)).pipe(
          tap(() => patchState({ keys: this.sort(context) } as M)),
          switchMap(() =>
            fetch
              ? dispatch(new ActionGet())
              : of(patchState({ loading: false } as M))
          ),
          map(() => patchState({ initialized: true } as M))
        );
  }
  ...
  public override get(context: StateContext<M>, action?: any): Observable<any> {
    ...
    return finishedPaging
      ...
      : of(null).pipe(
          map(() =>
            this.keysGet(context, keysFiltered).map((id: string) =>
          ...
          map((page: Array<QueryDocumentSnapshot<any>>) =>
            page
              .filter((document: QueryDocumentSnapshot<any>) => document.exists)
              .forEach((document: QueryDocumentSnapshot<any>) => {
                ...
                object.metadata =
                  object.metadata == null ? {} : object.metadata;
          ...
          switchMap(() => dispatch(new ActionFilter())),
          switchMap(() =>
            !fetchMedia
              ? of(null)
  ...
  protected sort(context: StateContext<M>): Array<string> {
    ...
    const keys: Array<string> = Object.keys(lookup).sort(
  ...
  public override data(context: StateContext<M>): Array<any> {
    ...
    const offset: number = keysFiltered.findIndex(
      (id: string) => lookup[id] == null
    );
    const end: number = offset === -1 ? keysFiltered.length : offset;

    return keysFiltered
      .slice(0, end)
      .map((id: string) => lookup[id])
  ...
  private keysGet(
    ...
    const keys: Array<string> =
      keysFiltered || StateChild.keysFilteredState(state);
