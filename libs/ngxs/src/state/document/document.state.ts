...
> {
  ...
  protected static snapshotState(state: any): FirestoreDocumentSnapshot<any> {
    return state.snapshot;
  }
  protected static formState(state: any): FormNgxs {
    return state.form;
  }
  protected static formGroupState(state: any): UntypedFormGroup {
    return state.formGroup;
  }
  protected static isFormState(state: any): boolean {
    return StateDocument.formGroupState(state) != null;
  }
  protected static dataState(state: any): any {
    return StateDocument.formState(state).model;
  }
  protected static idState(state: any): string {
    return StateDocument.dataState(state).id;
  }
  protected static isNewState(state: any): boolean {
    return StateDocument.idState(state) === CoreEnum.IdNew;
  }
  protected static canUpdateState(state: any): boolean {
    return (
      (StateDocument.formState(state).status === FormNgxsStatus.Valid &&
        StateDocument.formState(state).dirty) ||
      false
    );
  }
  protected static foundState(state: any): boolean {
    return Object.keys(StateDocument.dataState(state)).length > 0;
  }
  protected static metadataState(state: any): any {
    return StateDocument.dataState(state).metadata;
  }

  public static snapshot() {
    return createSelector([this], (state: any) =>
      StateDocument.snapshotState(state)
    );
  }
  public static form() {
    return createSelector([this], (state: any) =>
      StateDocument.formState(state)
    );
  }
  public static formGroup() {
    return createSelector([this], (state: any) =>
      StateDocument.formGroupState(state)
    );
  }
  public static isForm() {
    return createSelector([this], (state: any) =>
      StateDocument.isFormState(state)
    );
  }
  public static data() {
    return createSelector([this], (state: any) =>
      StateDocument.dataState(state)
    );
  }
  public static id() {
    return createSelector([this], (state: any) => StateDocument.idState(state));
  }
  public static isNew() {
    return createSelector([this], (state: any) =>
      StateDocument.isNewState(state)
    );
  }
  public static canUpdate() {
    return createSelector([this], (state: any) =>
      StateDocument.canUpdateState(state)
    );
  }
  public static found() {
    return createSelector([this], (state: any) =>
      StateDocument.foundState(state)
    );
  }
  ...
  public get(context: StateContext<M>, action: any): Observable<any> {
    ...
    return this.service
      .documentGet(this.collection, id)
      .pipe(
        switchMap((snapshot: FirestoreDocumentSnapshot<any>) =>
          dispatch([new ActionSet(snapshot), ...this.ActionsQueryAdd(snapshot)])
        )
      );
  }
  ...
  public watch(context: StateContext<M>, action: any): Observable<any> {
    ...
    return this.service.documentWatch<T>(this.collection, id).pipe(
      tap((snapshot: DocumentSnapshot<T>) => dispatch(new ActionSet(snapshot))),
  ...
  public set(context: StateContext<M>, action: any): Observable<any> {
    ...
    return dispatch(new ActionReset()).pipe(
      tap(() =>
        patchState({
          formGroup,
          snapshot
  ...
  public patch(context: StateContext<M>, action: any): Observable<any> {
    ...
    return dispatch([
      ...
      switchMap(() => dispatch(new SetFormDirty(path))),
      switchMap(() =>
        save
          ? this.service.documentUpdate(
              StateDocument.snapshotState(state),
              partial
            )
          : of(null)
  ...
  public save(context: StateContext<M>): Observable<any> {
    ...
    return dispatch(isNew ? new ActionCreate() : new ActionUpdate()).pipe(
  ...
  public delete(context: StateContext<M>): Observable<any> {
    ...
    const state: M = getState();
    const snapshot: FirestoreDocumentSnapshot<T> =
      StateDocument.snapshotState(state);
    ...
    return delete$.pipe(
      switchMap(() =>
        dispatch([new ActionReset(), ...this.ActionsQueryRemove(id)])
      ),
      map(() => !isNew)
    );
  }
  ...
  protected updateMedia(
    ...
    const control: AbstractControl = form
      .get('metadata')
      ?.get(imageType) as AbstractControl;
    ...
    return storage
      .storageUpload(dataUri, path)
      .pipe(
        tap((url: string) => this.patchMetadata(context, { [imageType]: url }))
      );
  }

  private ActionsReset(): Array<any> {
    const actions: Array<any> =
      this.actions.ActionsReset == null ? [] : this.actions.ActionsReset;

    return actions.map((ActionReset: any) => new ActionReset());
  }

  private ActionsCreate(): Array<any> {
    const actions: Array<any> =
      this.actions.ActionsCreate == null ? [] : this.actions.ActionsCreate;

    return actions.map((ActionCreate: any) => new ActionCreate());
  }

  private ActionsQueryAdd(
    snapshot: FirestoreDocumentSnapshot<any>,
    data?: T
  ): Array<any> {
    const actions: Array<any> =
      this.actions.ActionsQueryAdd == null ? [] : this.actions.ActionsQueryAdd;

    return actions.map(
      (ActionQueryAdd: any) => new ActionQueryAdd(snapshot, data)
    );
  }

  private ActionsQueryRemove(id: string): Array<any> {
    const actions: Array<any> =
      this.actions.ActionsQueryRemove == null
        ? []
        : this.actions.ActionsQueryRemove;

    return actions.map((ActionQueryRemove: any) => new ActionQueryRemove(id));
  }

  private ActionsQuerySync(data: T): Array<any> {
    const actions: Array<any> =
      this.actions.ActionsQuerySync == null
        ? []
        : this.actions.ActionsQuerySync;

    return actions.map((ActionQuerySync: any) => new ActionQuerySync(data));
  }
}
