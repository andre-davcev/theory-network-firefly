...
export class StateDevice {
  ...
  statusBarSet(
    { getState, patchState }: StateContext<StateDeviceModel>,
    { payload }: ActionDeviceStatusBarSet
  ) {
    ...
    const setStatusBarOptions: boolean =
      isDevice &&
      (optionsPrevious == null || options.style !== optionsPrevious.style);
  ...
  statusBarShow({ getState, patchState }: StateContext<StateDeviceModel>) {
    const state: StateDeviceModel = getState();
    const showStatusBar: boolean =
      StateDevice.device(state) && !StateDevice.statusBarVisible(state);
