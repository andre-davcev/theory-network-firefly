...
export class ComponentAuth implements OnInit {
  ...
  private id: UntypedFormControl = new UntypedFormControl(
    '',
    Validators.compose([
      Validators.required,
  ...
  private passwordLogin: UntypedFormControl = new UntypedFormControl(
    '',
    Validators.compose([
      Validators.required,
  ...
  private passwordSignUp: UntypedFormControl = new UntypedFormControl(
    '',
    Validators.compose([
      Validators.required,
      Validators.minLength(6),
  ...
  constructor(
    ...
    this.store.dispatch(new ActionUserSetErrorAuth()).pipe;
    this.translate
      .get([
        ...Object.values(AuthErrorId).map(
          (value: string) => `page.auth.errors.${AuthControl.Id}.${value}`
        ),
        ...Object.values(AuthErrorPassword).map(
          (value: string) => `page.auth.errors.${AuthControl.Password}.${value}`
        )
      ])
  ...
  public clicked(): void {
    ...
    this.store
      .dispatch(action)
      .pipe(
        filter(() => !this.store.selectSnapshot(StateUser.erroredAuth)),
        tap(() => this.finished.emit(true)),
        catchError(() => of(this.finished.emit(false)))
      )
      .subscribe();
  }
  ...
  private setErrorObservable(control: AuthControl): void {
    this.errors$[control] = combineLatest([
      ...
      map((errors: Array<string>) =>
        errors.map(
          (error: string) => this.errorDefinitions[control][error].message
        )
      )
    );

    this.hasErrors$[control] = this.errors$[control].pipe(
      map((errors: Array<string>) => errors.length > 0)
    );
  }
}
