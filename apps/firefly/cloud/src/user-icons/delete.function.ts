...
const UserIconsDelete: CloudFunction<DocumentSnapshot> =

firestore.
document('user-icons/{id}').
onDelete(async(snapshot: DocumentSnapshot, context: EventContext) =>
{
    const data:       Record<string, string>      = snapshot.data();
    const collection: CollectionReference         = database.collection('icons');
