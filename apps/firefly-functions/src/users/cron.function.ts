...
const UsersCron = runWith({ memory: '2GB', timeoutSeconds: 540 })
  .pubsub.schedule('0 3 * * *') // Every Day @ 3AM
  .onRun(async (context: EventContext) => {
    ...
    const chunks: Array<Array<string>> = list.users
      ...
      .map((user: auth.UserRecord) => user.uid)
      .reduce((chunks: Array<Array<string>>, uid: string, i: number) => {
        chunk = Math.floor(i / 10);

        chunks[chunk] = [...(chunks[chunk] || []), uid];

        return chunks;
      }, [])
      .slice(0, 10);

    chunks.forEach(async (chunk: Array<string>) => {
      await new Promise((resolve) => setTimeout(resolve, 1000));

      const deletions: Array<Promise<void>> = chunk.map((uid: string) =>
