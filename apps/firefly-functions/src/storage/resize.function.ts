...
const StorageResize = storage
  ...
  .onFinalize(async (object: storage.ObjectMetadata, context: EventContext) => {
    ...
    await sharp(filePathTemp)
      .resize({ width: fileSmallWidth, withoutEnlargement: true })
    ...
    const fileMediumDestination: string = join(
      dirname(filePath),
      fileMediumName
    );

    await sharp(filePathTemp)
      .resize({ width: fileMediumWidth, withoutEnlargement: true })
      .toFile(fileMediumPath);
